...
echo "     # Note: Prometheus auto-discovers Kyverno via pod annotations"
echo "     # Manual port-forward (use 8002 if 8000 is in use):"
echo "     kubectl port-forward -n kyverno svc/kyverno-svc-metrics 8002:8000"
echo "     curl http://localhost:8002/metrics"
echo ""
echo "  5. Apply Prometheus ServiceMonitor (if using Prometheus Operator):"
echo "     kubectl apply -f $KYVERNO_DIR/monitoring/prometheus-servicemonitor.yaml"
echo ""
